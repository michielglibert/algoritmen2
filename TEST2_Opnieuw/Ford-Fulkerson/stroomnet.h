...
template <class T>
Stroomnetwerk<T>::Stroomnetwerk(int grootte, int _van, int _naar) : Graaf<GERICHT>(grootte), GraafMetTakdata<GERICHT, T>(grootte), van(_van), naar(_naar) {}

template <class T>
Stroomnetwerk<T>::Stroomnetwerk(const GraafMetTakdata<GERICHT, T> &gr, int _van, int _naar) : Graaf<GERICHT>(gr), GraafMetTakdata<GERICHT, T>(gr), van(_van), naar(_naar) {}

template <class T>
Stroomnetwerk<T>::Stroomnetwerk(const Stroomnetwerk<T> &gr) : Graaf<GERICHT>(gr), GraafMetTakdata<GERICHT, T>(gr), van(gr.van), naar(gr.naar) {}
...
Stroomnetwerk<T> Stroomnetwerk<T>::geefStroom()
{
    ...
    while (vergrotendpad.size() != 0)
    {
        ...
        {
            int start = vergrotendpad[i - 1]; //start en eind van de tak
            eind = vergrotendpad[i];
            cout << start << " -> ";
        }
        cout << eind << endl;
...
Stroomnetwerk<T> &Stroomnetwerk<T>::operator-=(const Pad<T> &pad)
{
    ...
    for (int i = 1; i < pad.size(); i++)
    {
        ...
        else
        {
            ...
            cout << "Verbinding wordt aangepast naar " << this->takdatavector[taknr] << endl;
        }
...
{
    T padcapaciteit = pad.geefCapaciteit();
    cout << "-------Oplossing wordt aangepast met capaciteit " << padcapaciteit << endl;
    for (int i = 1; i < pad.size(); i++)
    {
        ...
        if (terugtak != -1)
        {
            cout << "Ik bekijk terugtak " << van << " naar " << naar << " met waarde " << this->takdatavector[terugtak] << endl;
            ...
            else
            {
                ...
                cout << "Verbinding wordt aangepast naar " << this->takdatavector[terugtak] << endl;
            }
        }
        else
        {
            cout << "Terugtak bestaat niet, vergroot tak" << endl;
