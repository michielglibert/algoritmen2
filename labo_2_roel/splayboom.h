...
class Splayboom : public Zoekboom<Sleutel,Data>{
	public:
		...
		void splay(Zoekboom<Sleutel,Data> *z);
...
template <class Sleutel,class Data>
void Splayboom<Sleutel,Data>::splay(Zoekboom<Sleutel,Data> *z){
	if(*z){
//		std::cout << "hallo ik ben knoop met sleutel" << (*z)->sleutel << std::endl;
		if((*z)->ouder == nullptr){
			...
			bool links = (*z)->ouder->links == (*z);
			if((*z)->ouder->ouder == nullptr){
				//zig
				Zoekboom<Sleutel,Data>::rotatie(*(*z)->ouder, links);
			}
			else{
				Zoekboom<Sleutel,Data> *ouder = (Splayboom<Sleutel,Data> *)Zoekboom<Sleutel,Data>::geefBoomBovenKnoop(*(*z)->ouder);
				bool ouderlinks = (*ouder)->ouder->links == (*ouder);
